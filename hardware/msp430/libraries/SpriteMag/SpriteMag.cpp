...
MagneticField SpriteMag::read() {
	...
	MagneticField b;
	b.x = ((int)m_receiveBuffer[0]<<8) + (int)m_receiveBuffer[1] + m_biasx;
	b.y = ((int)m_receiveBuffer[2]<<8) + (int)m_receiveBuffer[3] + m_biasy;
	b.z = ((int)m_receiveBuffer[4]<<8) + (int)m_receiveBuffer[5] + m_biasz;
